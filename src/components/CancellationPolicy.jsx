...
export default function CancellationPolicy() {
  ...
  const displayIntro = refundData?.introText || (!isLoaded ? "Our goal is to provide flexibility while ensuring fair scheduling for all guests." : null);

  const displaySections = (refundData?.sections && refundData.sections.length > 0)
    ? refundData.sections.map((section, idx) => ({
      id: idx + 1,
      title: section.title,
      content: section.points || []
    }))
    : (!isLoaded && !refundData?.content ? sections : []);

  const displayLastUpdated = refundData?.updatedAt
    ? `Last Updated: ${new Date(refundData.updatedAt).toLocaleDateString("en-US", {
      day: "numeric",
      month: "long",
      year: "numeric",
    })}`
    : refundData?.createdAt
      ? `Last Updated: ${new Date(refundData.createdAt).toLocaleDateString("en-US", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })}`
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {isLoaded && !refundData ? (
          <div className="bg-theatre-grey-deep/30 border border-theatre-gold/25 rounded-2xl p-12 text-center text-gray-400 font-sans text-base">
            Cancellation & Refund Policy is currently unavailable.
          </div>
        ) : (
          <>
            {/* Introduction */}
