...
export default function BookNow({ selectedEventName, clearSelectedEvent }) {
  ...
  const [wantsDecor, setWantsDecor] = useState(null);
  ...
  const toggleScreenDesc = (screenId, e) => {
    e.stopPropagation();
  ...
  // Added States for refactored flow
  const [validDates, setValidDates] = useState([]);
  const [currentMonth, setCurrentMonth] = useState(new Date());
  const [activeAccordion1, setActiveAccordion1] = useState('screen');
  const [activeAccordion2, setActiveAccordion2] = useState('occasion');
  const [activeAccordion3, setActiveAccordion3] = useState('addons');
  ...
  useEffect(() => {
    const fetchValidDates = async () => {
      ...
      try {
        // Fetch all slots for this screen to find which dates have slots
        const res = await getSlots(selectedScreenId, '');
        if (res && res.data) {
          const dates = res.data
            .map(slot => slot.date ? slot.date.split('T')[0] : null)
            .filter(Boolean);
  ...
  useEffect(() => {
    const fetchTimeSlots = async () => {
      if (!selectedScreenId || !selectedDate || screens.length === 0) return;

      try {
        setLoadingSlots(true);
        const res = await getSlots(selectedScreenId, selectedDate);
  ...
  }, [selectedDate, screens, selectedScreenId]);
  ...
  const handleNextStep = () => {
    ...
    if (activeStep === 1) {
      ...
      const highestCategory = [...activeCategories].sort((a, b) => b.from - a.from)[0];
      const highestCategoryCount = highestCategory ? (guestCounts[highestCategory._id] || 0) : 0;

      if (totalGuests === 0) {
        errors.guests = 'Please select at least 1 guest.';
      } else if (highestCategory && highestCategoryCount < 1) {
        const catName = highestCategory.name || (highestCategory.to >= 100 ? "Adults" : `Kids`);
        errors.guests = `At least 1 member must be selected in the highest age category (${catName}).`;
    ...
    if (activeStep === 2) {
      if (!eventCategory) errors.occasion = 'Please select an occasion to continue.';
      if (wantsCake === null) errors.cake = 'Please select whether you need a cake.';
      if (screenDecorations.length > 0 && wantsDecor === null) errors.decor = 'Please select whether you need decoration.';
  ...
  // Check if all required selections for a given step are satisfied
  const isStepValid = (stepNum) => {
    if (stepNum === 1) {
      const highestCategory = [...activeCategories].sort((a, b) => b.from - a.from)[0];
      const highestCategoryCount = highestCategory ? (guestCounts[highestCategory._id] || 0) : 0;
      const hasHighest = highestCategory ? highestCategoryCount > 0 : true;

      return !!selectedScreenId && !!selectedDate && !!selectedTimeSlot && totalGuests > 0 && totalGuests <= maxCapacity && hasHighest;
    }
    if (stepNum === 2) {
      const decorOk = screenDecorations.length === 0 || wantsDecor !== null;
      return !!eventCategory && wantsCake !== null && decorOk;
    }
    return true;
  };
  ...
  return (
    <section id="book-now" className={`relative bg-theatre-dark/95 overflow-x-hidden transition-all duration-500 ${activeStep === 6 ? 'flex items-center justify-center h-[calc(100dvh-80px)] sm:h-[calc(100vh-80px)] overflow-hidden m-0 p-0' : 'min-h-screen pt-16 pb-20 sm:py-16'
      ...
      <div className="w-full max-w-full mx-auto px-2 sm:px-8 lg:px-12 relative z-10">
        {activeStep <= 5 && (
          <div ref={stepperScrollRef} className="w-full max-w-5xl mx-auto mb-12 overflow-x-auto pb-4 no-scrollbar">
                  ...
                  <div key={name} className="flex items-center flex-grow last:flex-grow-0">
                    <div className="flex flex-col items-center relative">
                      <div
                        ...
                        className={`w-9 h-9 rounded-full flex items-center justify-center font-sans text-xs font-bold transition-all duration-300 border cursor-pointer ${isCompleted
                          ...
                          : isActive
                            ...
                            : 'bg-theatre-grey-deep/30 border-white/10 text-gray-500 hover:border-white/30'
                          }`}
                      >
                      ...
                      <span
                        ...
                        className={`text-[11px] mt-2 font-medium tracking-wide uppercase transition-colors duration-300 text-center whitespace-nowrap cursor-pointer ${isActive ? 'text-theatre-gold font-bold' : isCompleted ? 'text-theatre-gold/70 hover:text-theatre-gold' : 'text-gray-500 hover:text-gray-300'
                        }`}>
                        {name}
                      </span>
                    </div>
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-12 gap-10 items-start max-w-6xl mx-auto ${activeStep === 6 ? 'm-0 w-full' : 'mb-4'}`}>
          ...
          <div className={`col-span-1 bg-theatre-grey-deep/20 backdrop-blur-md border border-white/5 rounded-3xl flex flex-col justify-between transition-all duration-300 mx-0 ${
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeStep > 1 && getFirstInvalidPrerequisite(activeStep) !== null && (() => {
                  ...
                  return (
                    <div className="flex flex-col items-center justify-center min-h-[320px] space-y-6 max-w-md mx-auto w-full py-8">
                      <div className="w-16 h-16 rounded-full bg-red-500/10 border border-red-500/30 flex items-center justify-center">
                        <AlertCircle className="w-8 h-8 text-red-400" />
                      </div>
                      <div className="text-center space-y-2">
                        <h4 className="text-white font-bold text-lg font-serif">Step {failedStep} Incomplete</h4>
                        <p className="text-gray-400 text-sm leading-relaxed">
                          Please complete <span className="text-theatre-gold font-bold">Step {failedStep}: {failedStepName}</span> before proceeding to this step.
                        </p>
                      </div>
                      <button
                        type="button"
                        ...
                        className="inline-flex items-center space-x-2 bg-theatre-gold hover:bg-theatre-gold-light text-theatre-grey-deep font-sans text-xs font-bold px-6 py-3 rounded-xl shadow-md transition-all duration-300 cursor-pointer"
                      >
                        <ChevronLeft className="w-4 h-4 text-theatre-grey-deep" />
                        <span>Go to Step {failedStep}: {failedStepName}</span>
                      </button>
                    </div>
                  );
                })()}
                ...
                {activeStep === 1 && (
                  <div className="space-y-8 max-w-3xl mx-auto w-full">
                    ...
                    <div className="border border-white/10 rounded-2xl overflow-hidden bg-theatre-dark/30 mb-6">
                      <div
                                ...
                                {screens && screens.length > 0 ? screens.map((screen) => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      onClick={() => { setSelectedScreen(screen.name); setSelectedScreenId(screen._id); setSelectedTimeSlot(''); setSelectedSlotId(null); setStepErrors({}); setActiveAccordion1('slot'); }}
                                    ...
                                    >
                                      ...
                                      <div className="p-4 flex-grow flex flex-col justify-between space-y-3">
                                        <div className="space-y-1">
                                          ...
                                          {screen.description && (() => {
                                            const isExpanded = !!expandedScreens[screen._id];
                                            const isLong = screen.description.length > 80;
                                            return (
                                              <div>
                                                <p className={`text-xs text-gray-400 font-light leading-relaxed ${!isExpanded && isLong ? 'line-clamp-2' : ''}`}>
                                                  {screen.description}
                                                </p>
                                                {isLong && (
                                                  <button
                                                    type="button"
                                                    onClick={(e) => toggleScreenDesc(screen._id, e)}
                                                    className="text-theatre-gold text-[10px] font-bold mt-1 hover:text-theatre-gold-light transition-colors cursor-pointer"
                                                  >
                    ...
                    <div className="border border-white/10 rounded-2xl overflow-hidden bg-theatre-dark/30 mb-6">
                      <div
                        ...
                        className="bg-white/5 px-3 sm:px-6 py-4 cursor-pointer flex justify-between items-center hover:bg-white/10 transition-colors"
                      >
                      ...
                      <AnimatePresence initial={false}>
                        {activeAccordion1 === 'slot' && (
                          ...
                          >
                            <div className="p-3 sm:p-6 border-t border-white/5 space-y-4">
                              {!selectedScreenId ? (
                                <div className="p-8 text-center text-red-400 font-bold bg-black/40 rounded-xl border border-red-500/30 flex flex-col items-center justify-center">
                                  <AlertCircle className="w-8 h-8 mx-auto mb-3" />
                                  <p>Please select a screening hall first</p>
                                </div>
                              ) : (
                ...
                {activeStep === 2 && getFirstInvalidPrerequisite(activeStep) === null && (
                  <div className="space-y-8 max-w-3xl mx-auto w-full">
                    <div className="space-y-1 text-center py-2 border-b border-white/5 pb-4">
                      ...
                      <AnimatePresence initial={false}>
                        {activeAccordion2 === 'occasion' && (
                          <motion.div
                            ...
                            className="overflow-hidden"
                          >
                            <div className="p-3 sm:p-8 pt-6 sm:pt-8 border-t border-white/5 space-y-6">
                              {/* 2.1: Occasions Grid */}
                              <div className="space-y-6">
                                ...
                                {(() => {
                                  ...
                                  const chunks = [];
                                  for (let i = 0; i < occasionList.length; i += 15) {
                                    chunks.push(occasionList.slice(i, i + 15));
                                  }
                                          ...
                                          <div key={chunkIdx} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 min-w-full snap-start">
                                            {chunk.map(cat => {
                                              ...
                                              return (
                                                <div
                                                  key={cat.name}
                                                  onClick={() => { setEventCategory(cat.name); setActiveAccordion2('cake'); }}
                          ...
                          >
                            <div className="p-3 sm:p-8 pt-6 sm:pt-8 border-t border-white/5 space-y-6">
                              {!eventCategory ? (
                                <div className="p-8 text-center text-red-400 font-bold bg-black/40 rounded-xl border border-red-500/30 flex flex-col items-center justify-center">
                                  <AlertCircle className="w-8 h-8 mx-auto mb-3" />
                                  <p>Please select an occasion first</p>
                                </div>
                              ) : (
                                <>
                                  {/* 2.2: Cake Selection */}
                                ...
                                <div className="flex items-center space-x-3">
                                  <button
                                    type="button"
                                    onClick={() => { setWantsCake(true); setStepErrors(prev => ({ ...prev, cake: null })); }}
                                    className={`px-5 py-2.5 rounded-xl border text-xs font-sans font-bold tracking-wider transition-all duration-300 cursor-pointer ${wantsCake === true
                                      ? 'bg-theatre-gold border-theatre-gold text-theatre-grey-deep'
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => { setWantsCake(false); setStepErrors(prev => ({ ...prev, cake: null })); setActiveAccordion2('decor'); }}
                                    className={`px-5 py-2.5 rounded-xl border text-xs font-sans font-bold tracking-wider transition-all duration-300 cursor-pointer ${wantsCake === false
                                      ? 'bg-theatre-gold border-theatre-gold text-theatre-grey-deep'
                                      : 'bg-white/5 border-white/10 text-gray-300'
                                      }`}
                                  >
                                ...
                                {stepErrors.cake && (
                                  <div className="text-red-400 text-xs">{stepErrors.cake}</div>
                                )}
                                ...
                                {wantsCake && (
                                  ...
                                  >
                                      ...
                                      <div className="space-y-4">
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                          {(() => {
                                            ...
                                            const chunks = [];
                                            for (let i = 0; i < cakeList.length; i += 15) {
                                    ...
                                    <div className="flex justify-end pt-2">
                                      <button
                                        type="button"
                                        onClick={() => setActiveAccordion2('decor')}
                                        className="bg-theatre-gold hover:bg-theatre-gold-light text-theatre-grey-deep font-sans text-xs font-bold px-6 py-2.5 rounded-xl shadow-md transition-all duration-300 cursor-pointer"
                                      >
                                        Next
                                      </button>
                                    </div>
                          ...
                          >
                            <div className="p-3 sm:p-8 pt-6 sm:pt-8 border-t border-white/5 space-y-6">
                              {wantsCake === null ? (
                                <div className="p-8 text-center text-red-400 font-bold bg-black/40 rounded-xl border border-red-500/30 flex flex-col items-center justify-center">
                                  <AlertCircle className="w-8 h-8 mx-auto mb-3" />
                                  <p>Please select a cake option first</p>
                                </div>
                              ) : (
                                <>
                                ...
                                <div className="space-y-6">
                                  ...
                                  <div className="flex items-center space-x-3">
                                    <button
                                      type="button"
                                      onClick={() => { setWantsDecor(true); setStepErrors(prev => ({ ...prev, decor: null })); }}
                                      className={`px-5 py-2.5 rounded-xl border text-xs font-sans font-bold tracking-wider transition-all duration-300 cursor-pointer ${wantsDecor === true
                                        ? 'bg-theatre-gold border-theatre-gold text-theatre-grey-deep'
                                        : 'bg-white/5 border-white/10 text-gray-300'
                                        }`}
                                    >
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => { setWantsDecor(false); setStepErrors(prev => ({ ...prev, decor: null })); }}
                                      className={`px-5 py-2.5 rounded-xl border text-xs font-sans font-bold tracking-wider transition-all duration-300 cursor-pointer ${wantsDecor === false
                                        ? 'bg-theatre-gold border-theatre-gold text-theatre-grey-deep'
                                        : 'bg-white/5 border-white/10 text-gray-300'
                                        }`}
                                    >
                                  ...
                                  {stepErrors.decor && (
                                    <div className="text-red-400 text-xs">{stepErrors.decor}</div>
                                  )}
                                    ...
                                    >
                                      {loadingDecorations ? (
                                        ...
                                        <div className="flex overflow-x-auto snap-x snap-mandatory gap-6 pt-2 pb-4 px-1 no-scrollbar w-full">
                                          {(() => {
                                            const chunks = [];
                                            for (let i = 0; i < screenDecorations.length; i += 15) {
                                              chunks.push(screenDecorations.slice(i, i + 15));
                                            }
                                                ...
                                                {chunk.map((decor) => {
                                                  ...
                                                  return (
                                                    ...
                                                    >
                                                      ...
                                                      <div className="h-24 sm:h-28 bg-gray-900 overflow-hidden flex items-center justify-center">
                                                        {decor.image && decor.image.path ? (
                                                          <img 
                                                            src={getImageUrl(decor.image.path)} 
                                                            alt={decor.name}
                                                            className="w-full h-full object-cover"
                                                          />
                                                        ) : (
                                                          <Gift className="w-8 h-8 text-theatre-gold opacity-50" />
                                                        )}
                                                      </div>
                                                      <div className="pt-2 pb-2 px-2 text-center space-y-0.5">
                                                        <h4 className="text-[11px] font-bold text-white line-clamp-2" title={decor.name}>{decor.name}</h4>
                                                        <span className="text-xs text-theatre-gold font-bold">₹{formatCurrency(decor.price)}</span>
                                                      </div>
                                                    </div>
                                                  );
                                                })}
                ...
                {activeStep === 3 && getFirstInvalidPrerequisite(activeStep) === null && (
                            ...
                            <div className="p-3 sm:p-8 pt-6 sm:pt-8 border-t border-white/5 space-y-6">
                              <div className="flex overflow-x-auto snap-x snap-mandatory gap-6 pt-2 pb-4 px-1 no-scrollbar w-full">
                                {(() => {
                                  ...
                                  const chunks = [];
                                  for (let i = 0; i < addonList.length; i += 15) {
                                    chunks.push(addonList.slice(i, i + 15));
                                  }
                                  return chunks.map((chunk, chunkIdx) => (
                                    <div key={chunkIdx} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 min-w-full snap-start">
                                      {chunk.map(addon => {
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            <div className="p-3 text-center flex-grow flex flex-col justify-between">
                                              <h4 className="text-[11px] font-bold text-white mb-1.5 line-clamp-2 leading-tight">{addon.name}</h4>
                                              <span className="text-xs text-theatre-gold font-bold">{addon.price}</span>
                                            </div>
                ...
                {activeStep === 4 && getFirstInvalidPrerequisite(activeStep) === null && (
                  <div className="space-y-8 max-w-3xl mx-auto w-full">
                    <div className="space-y-1 text-center py-2 border-b border-white/5 pb-4">
                ...
                {activeStep === 5 && getFirstInvalidPrerequisite(activeStep) === null && (
                  <div className="space-y-8 max-w-3xl mx-auto w-full">
                    <div className="space-y-1 text-center py-2 border-b border-white/5 pb-4">
            ...
            {refundPolicyData && refundPolicyData.content && activeStep <= 5 && (
